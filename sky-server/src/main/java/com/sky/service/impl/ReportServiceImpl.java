...
public class ReportServiceImpl implements ReportService {
    ...
    public TurnoverReportVO getTurnoverStatistics(LocalDate begin, LocalDate end) {
        ...
        for (LocalDate date : dateList) {
            //查询date日期对应的营业额数据，营业额是指：状态为已完成的订单的金额合计
    ...
    public UserReportVO getUserStatistics(LocalDate begin, LocalDate end) {
        ...
        //每天新增用户数量 select count(id) from user crate_time < ? and crate_time > ?
        List<Integer> newUserList = new ArrayList<>();
        //存放每天的总用户数量 select count(id) from user crate_time < ?
     ...
     * @param response
     */
    public void exportBusunessData(HttpServletResponse response) {
        // 1. 查询数据库，获取营业数据--查询
